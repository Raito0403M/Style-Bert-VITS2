"""
Streaming API server for TTS
WAVファイルをチャンク単位でストリーミング送信する実装
"""
...
def wav_streaming_generator(sr: int, audio: bytes, chunk_size: int = 8192) -> Generator[bytes, None, None]:
    """
    WAVファイルをチャンク単位でストリーミング送信するジェネレーター
...
if __name__ == "__main__":
    ...
    async def voice(
        ...
        """Infer text to speech(テキストから感情付き音声を生成する) - 通常の一括送信版"""
        logger.info(
    ...
    @app.api_route("/voice/stream", methods=["GET", "POST"])
    async def voice_stream(
        ...
        chunk_size: int = Query(8192, description="ストリーミングチャンクサイズ（バイト）"),
    ):
        """Infer text to speech with streaming(テキストから感情付き音声を生成してストリーミング送信する)"""
        logger.info(
        ...
        logger.success("Audio data generated, starting streaming...")
        
        # ストリーミングレスポンスを返す
        return StreamingResponse(
            wav_streaming_generator(sr, audio, chunk_size),
    ...
    logger.info("Streaming endpoint available at: /voice/stream")
    uvicorn.run(
